...
###  老师您好。我有一个困扰很久的问题可以请教你一下吗？就是同一个事物里先修改 再查询 为什么事物没提交(单个方法没走完)却能查询到修改之后的数据呢 

在数据库事务是指作为单个逻辑工作线程单元中执行的一系列操作,要么完全地执行,要么完全地不执行，
这是一种机制，用以维护数据库的完整性。
在你这个事务单元中存在两个操作，一个是update，一个是select；
你前面的update执行成功了，再执行select肯定能够查询到前面update的数据，
因为你是在一个线程单元中。但是另外一个事务（即另一个线程单元中）则不能select到这个update的数据，
因为你update的数据还没提交，只在该线程单元中中有效。

如果你对事务还不太熟悉，建议阅读：[事务](db/transaction.md)

这里有提到 事务的隔离性指的是两个事务之间，在同一事务中不存在隔离，先修改再取就会是改后的数据，即使没有commit
